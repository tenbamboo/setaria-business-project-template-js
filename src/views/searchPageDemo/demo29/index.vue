<template>
  <div>
    <!--
   通过本Demo可了解控制表单hidden unhidden

    在有的场景下，我们需要通过某些前置数据，来控制接下来的数据是否hidden unhidden

    这时，我们可以使用框架自带的 addFormItemHidden removeFormItemHidden
    比如以下场景：
    当用户新增数据时，
...
<script setup name="searchPageDemo29">
  ...
  const handlerSexChange = (list) => {
    if (list) {
      if (list[0].fldValue === "1") {
        addFormItemHidden(tableUiSchema, [
          "userName", // 用户号
          "userNum", // 用户分类
        ]);
        removeFormItemHidden(tableUiSchema, [
          "workDate", // 工作时间
          "whomodified", // 编辑人
        ]);
      } else {
